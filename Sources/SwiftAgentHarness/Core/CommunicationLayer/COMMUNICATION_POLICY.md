...
- **Mutation-time tenancy (REST + WebSocket):** When strict authenticated tenancy is enabled (`TenancyPolicySettings.requireAuthenticatedOwnerOnMutations`), inbound REST handlers and mutating WebSocket frames require a resolved **`APISessionContext.authenticatedOwnerAccountID`** from a validated **`Authorization: Bearer`** harness access JWT (`sub` = owner UUID) and verify it against the persisted conversation’s **`ownerAccountID`** for the targeted conversation id. The legacy **`X-SAH-Authenticated-Owner`** header is **ignored**. This complements subscribe-time checks above; keep the same Bearer token on **`/api`** and **`/ws`** upgrade for the same principal.
