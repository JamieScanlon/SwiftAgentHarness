...
- **Subscribe-time authorization** for multiplexed harness topics is enforced in [`WebSocketTopicSubscriptionRouter`](WebSocketTopicSubscriptionRouter.swift) via [`WebSocketTopicSubscribeAuthorization`](WebSocketTopicSubscribeAuthorization.swift):
  ...
  - **`trace/server`**: process-wide observability stream. Operator-scoped by default (`ServerConfig/enforceOperatorForServerTraceSubscribe`, default **`true`**); subscribe requires ``X-SAH-Authenticated-Owner`` matching an entry in ``ServerConfig/serverTraceOperatorOwnerIDs``. Also auto-engaged when ``requireAuthenticatedTenantOnAPI`` is true. Set ``enforceOperatorForServerTraceSubscribe`` to **`false`** for single-tenant open subscribe.
...
- **Mutation-time tenancy (REST + WebSocket):** When strict authenticated tenancy is enabled (`TenancyPolicySettings.requireAuthenticatedOwnerOnMutations`), inbound REST handlers and mutating WebSocket frames require a resolved **`APISessionContext.authenticatedOwnerAccountID`** (typically **`X-SAH-Authenticated-Owner`**) and verify it against the persisted conversation’s **`ownerAccountID`** for the targeted conversation id. This complements subscribe-time checks above; keep headers consistent across **`/api`** and **`/ws`** for the same principal.
