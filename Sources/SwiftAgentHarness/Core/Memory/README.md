...
**Exclusive plugin slot (S1).** One active `MemoryCapability` owns recall, promotion, flush policy, and runtime hooks. `DefaultMemoryService` is an orchestrator: project-instruction loading, flush trigger/dedupe/write-guard, and write tracking stay in the service; everything else delegates to the active capability's slots. Default registration is `builtin-file` (`FileStoreMemoryBackend`). `registerActiveMemoryCapability(_:)` or `registerExternalMemoryProvider(id:provider:)` **replaces** the active backend (legacy `MemoryProviding` adapter is test/transition-only — hook-only, no recall ownership). Pre-compaction flush **policy** lives in `MemoryFlushPlanResolving`; flush **execution** (spawn, timeout, write guard) stays in the service.
