...
**User-tier memory.** Cross-project facts live at `<configHome>/memory/user/` (`AgentMemoryPathResolver.resolveUserMemoryDirectory`). `type: user` memories route to the user tier at write time (`user/` path prefix in memory-write subagents; bare filenames stay project-scoped). Session prompts inject the user-tier `MEMORY.md` snapshot first (50 lines / 8 KB cap), then the project index (200 lines / 25 KB) — project wins on conflict. Turn recall merges both tiers in `manifestEntries` with `[scope:user]` / `[scope:project]` tags; recalled topic bodies use the same scope labels with user tier first and project tier last (nearest-wins).

**Strict-tenancy owner partition.** When `TenancyPolicySettings.requireAuthenticatedOwnerOnMutations` is true (`ServerConfig.requireAuthenticatedTenantOnAPI`), memory paths include the conversation owner as the outer address segment: project memory at `<configHome>/owners/<ownerUUID>/projects/<projectKey>/memory/` and user memory at `<configHome>/owners/<ownerUUID>/memory/user/`. Extraction, recall, injection, and dreaming sweeps all use the owner-scoped directories resolved from `conversation.ownerAccountID` via `DefaultMemoryService.makeSessionContext`. `makeSessionContext` fails closed when strict tenancy is on but the owner is missing. Single-tenant / non-strict deployments keep the legacy layout (`<configHome>/projects/<projectKey>/memory/`, `<configHome>/memory/user/`). `SAH_MEMORY_PATH_OVERRIDE` bypasses layout entirely (operator escape hatch; owner partitioning is not applied). Enabling strict tenancy does **not** migrate existing unscoped memory — prior content under legacy paths is orphaned until manually moved.

**Cross-project promotion (deferred).** A consolidation-style promotion lane — recurring facts lifted from two or more project stores into the user tier, approval-gated, with per-project sources retired — is **not implemented**. The cross-project transfer spec marks that lane as design synthesis, not observed behavior. Write-time type routing and read-time scope layering (above) are the supported cross-project mechanisms today. A future promotion sweep would depend on extending the per-project recall store (`memory/.dreams/recalls.jsonl`) for **multi-project recurrence** (project-keyed traces and cross-store aggregation), not only the within-project multi-day signals that dreaming consolidation uses now.
