...
public enum ProviderManifestLoader {
    public static func bundledManifestData(for providerID: ProviderID) -> Data? {
        let candidates = [
            ...
            ProviderResourceBundle.resourceBundle.url(forResource: providerID, withExtension: "manifest.json"),
        ]
        guard let url = candidates.compactMap({ $0 }).first else { return nil }
        return try? Data(contentsOf: url)
    }

    public static func decodeBundledManifest(for providerID: ProviderID) throws -> ProviderManifest? {
