...
/// Serializes access to the process-global ``WorkspaceMirrorSync/shared`` during tests.
///
/// Uses an explicit waiter queue so isolation remains exclusive across `await` points.
/// A plain actor method is insufficient: Swift actors are reentrant, so two tests can
/// interleave inside `run` while one is suspended in `syncBefore` / `finalizeExec`.
enum WorkspaceMirrorSyncTestIsolation {
    private actor Gate {
