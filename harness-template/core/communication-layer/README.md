...
**Persisted Checkpoint envelopes** (appends to `derivedEvents`). These also survive restart and appear in `read(id, { includeDerived: true })`. Concrete kinds: `CompactionCheckpoint`, `MemoryInjectionSnapshot`, `ToolResultTrim`, `SystemPromptAssembly`. The Context Engine's work. Visible to clients that want to render "compaction is running," "what summary did the model see," or transparency UIs. Hidden by default in conversational chat UIs.
