...
- **Writes to the Conversation Manager** — Checkpoints into `derivedEvents` via `appendCheckpoint(...)`. The Engine is the *sole* writer of the Checkpoint kinds (CompactionCheckpoint, MemoryInjectionSnapshot, ToolResultTrim, optional SystemPromptAssembly). It never writes to `rawEvents`.
