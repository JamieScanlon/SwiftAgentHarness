...
ACTUAL_LINK="$(readlink "$ROOT/Sources/SwiftAgentHarness/openapi.yaml" || true)"
if [ "$ACTUAL_LINK" != "$EXPECTED_LINK" ]; then
  echo "SwiftAgentHarness/openapi.yaml must be symlinked to $EXPECTED_LINK (found: $ACTUAL_LINK)" >&2
  exit 1
fi

python3 - "$ROOT/openapi/schemas/ws" "$ROOT/Sources/SwiftAgentHarness/Resources/WSSchemas" <<'PY'
