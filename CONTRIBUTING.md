...
4. **Include tests.** All new code must come with tests. Tests must not depend on shared state and should run in parallel in any order. The full suite should finish in under a minute — a longer run usually means a hung test.
